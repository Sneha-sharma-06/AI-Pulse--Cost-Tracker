...
router.post('/', auth, (req, res) => {
  try {
    ...
    const result = run(
      'INSERT INTO budget_alerts (user_id, team_id, project_id, threshold_percent, alert_type, notify_users) VALUES (?, ?, ?, ?, ?, ?)',
      [req.user.id, team_id || null, project_id || null, threshold_percent || 80, alert_type || 'email', notify_users || '']
    );
...
router.put('/:id', auth, (req, res) => {
  try {
    ...
    run(
      'UPDATE budget_alerts SET threshold_percent = ?, alert_type = ?, is_active = ?, notify_users = ? WHERE id = ? AND user_id = ?',
      [threshold_percent, alert_type, is_active, notify_users, req.params.id, req.user.id]
    );
...
router.get('/notifications', auth, (req, res) => {
  try {
    ...
    const notifications = all(`
      SELECT ah.*, ba.threshold_percent
      FROM alert_history ah
      LEFT JOIN budget_alerts ba ON ah.alert_id = ba.id
      WHERE ah.user_id = ?
...
// Check budget alerts (called by cron)
router.post('/check', auth, (req, res) => {
  try {
    const alerts = all(`
      SELECT ba.*,
        COALESCE((SELECT SUM(total_cost) FROM api_logs WHERE user_id = ba.user_id AND strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')), 0) as current_spend
      FROM budget_alerts ba
    ...
    for (const alert of alerts) {
      const user = get('SELECT id FROM users WHERE id = ?', [alert.user_id]);
      if (!user) continue;
      ...
      if (percentUsed >= alert.threshold_percent) {
        ...
        run('INSERT INTO alert_history (alert_id, user_id, message, severity) VALUES (?, ?, ?, ?)',
          [alert.id, alert.user_id, message, percentUsed >= 100 ? 'critical' : 'warning']);
