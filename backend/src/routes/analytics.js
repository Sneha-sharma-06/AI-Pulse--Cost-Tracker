...
// Get token efficiency by prompt
...
router.get('/forecast', auth, (req, res) => {
  try {
    ...
    const week1Cost = get(`
      ...
      FROM api_logs
      WHERE user_id = ? AND created_at >= datetime('now', '-14 days') AND created_at < datetime('now', '-7 days')
    `, [userId]);
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      forecast.push({
        ...
        projected_cost: parseFloat(dayCost.toFixed(4)),
        cumulative_cost: parseFloat(projectedCost.toFixed(4))
      });
    }

    res.json({
      current_avg_daily: parseFloat(avgDailyCost.toFixed(4)),
      growth_rate: parseFloat(growthRate.toFixed(2)),
      forecast_30_days: parseFloat(projectedCost.toFixed(2)),
  ...
  try {
    ...
    const comparison = all(`
      SELECT
        ...
        AVG(latency_ms) as avg_latency,
        AVG(total_cost / NULLIF(total_tokens, 0) * 1000) as cost_per_1k_tokens,
        CAST(SUM(CASE WHEN status = 'success' THEN 1 ELSE 0 END) AS FLOAT) * 100.0 / COUNT(*) as success_rate
      FROM api_logs
      WHERE user_id = ? AND created_at >= datetime('now', '-' || ? || ' days')
      GROUP BY model
