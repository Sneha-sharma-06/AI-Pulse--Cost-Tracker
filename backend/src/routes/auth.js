...
router.post('/register', async (req, res) => {
  try {
    ...
    run('INSERT INTO audit_logs (user_id, action, resource_type, resource_id, details) VALUES (?, ?, ?, ?, ?)', [userId, 'create', 'user', userId, `Account created: ${username}`]);

    const token = jwt.sign({ id: userId, username }, process.env.JWT_SECRET, { expiresIn: '7d' });
    res.status(201).json({ message: 'Account created', token, user: { id: userId, username, name, role: 'user', created_at: new Date().toISOString() } });
  } catch (error) {
    console.error(error);
...
router.post('/login', async (req, res) => {
  try {
    ...
    run('INSERT INTO audit_logs (user_id, action, resource_type, details) VALUES (?, ?, ?, ?)', [user.id, 'login', 'user', 'Logged in from browser']);
    const token = jwt.sign({ id: user.id, username: user.username }, process.env.JWT_SECRET, { expiresIn: '7d' });
    res.json({ message: 'Logged in', token, user: { id: user.id, username: user.username, name: user.name, role: user.role, created_at: user.created_at } });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Server error' });
  }
