...
router.get('/csv', auth, (req, res) => {
  try {
    const { start_date, end_date, model, provider, project_id } = req.query;
    ...
    const csvRows = logs.map(log =>
      `${log.id},"${log.created_at}","${log.model}","${log.provider}","${log.endpoint}",${log.prompt_tokens},${log.completion_tokens},${log.total_tokens},${log.total_cost},${log.latency_ms},"${log.status}"`
    ).join('\n');
...
router.get('/summary', auth, (req, res) => {
  try {
    ...
    const topEndpoints = all(`
      SELECT endpoint,
        COUNT(*) as calls,
        AVG(total_cost) as avg_cost,
        AVG(latency_ms) as avg_latency
      FROM api_logs
      WHERE user_id = ? AND created_at >= datetime('now', '-' || ? || ' days') AND endpoint IS NOT NULL
