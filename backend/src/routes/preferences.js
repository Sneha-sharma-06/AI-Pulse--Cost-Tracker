...
router.get('/', auth, (req, res) => {
  try {
    ...
    if (!prefs) {
      run('INSERT INTO user_preferences (user_id) VALUES (?)', [req.user.id]);
      prefs = get('SELECT * FROM user_preferences WHERE user_id = ?', [req.user.id]);
    }
    res.json(prefs);
  } catch (error) {
    console.error(error);
...
router.put('/', auth, (req, res) => {
  try {
    ...
    run(
      'UPDATE user_preferences SET theme = ?, language = ?, timezone = ?, email_notifications = ?, budget_alert_enabled = ?, default_date_range = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?',
      [theme, language, timezone, email_notifications, budget_alert_enabled, default_date_range, req.user.id]
    );
