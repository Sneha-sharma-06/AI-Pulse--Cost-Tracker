...
router.get('/', auth, (req, res) => {
  try {
    const projects = all(`
      SELECT p.*,
        (SELECT COUNT(*) FROM api_logs WHERE project_id = p.id) as total_calls,
        (SELECT COALESCE(SUM(total_cost), 0) FROM api_logs WHERE project_id = p.id AND strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')) as monthly_cost
      FROM projects p
...
router.post('/', auth, (req, res) => {
  try {
    ...
    const result = run(
      'INSERT INTO projects (user_id, team_id, name, description, budget_limit, color) VALUES (?, ?, ?, ?, ?, ?)',
      [req.user.id, null, name, description, budget_limit, color || '#0ea5e9']
    );
...
router.put('/:id', auth, (req, res) => {
  try {
    ...
    run(
      'UPDATE projects SET name = ?, description = ?, budget_limit = ?, color = ?, is_active = ? WHERE id = ? AND user_id = ?',
      [name, description, budget_limit, color, is_active, req.params.id, req.user.id]
    );
...
router.get('/:id/costs', auth, (req, res) => {
  try {
    ...
    const dailyCosts = all(`
      SELECT DATE(created_at) as date,
        SUM(total_cost) as cost,
        SUM(total_tokens) as tokens,
        COUNT(*) as calls
      FROM api_logs
      WHERE project_id = ? AND user_id = ? AND created_at >= datetime('now', '-' || ? || ' days')
