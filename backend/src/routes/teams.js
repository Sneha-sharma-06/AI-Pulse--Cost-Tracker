  ...
  try {
    const teams = all(`
      SELECT t.*, u.name as owner_name,
        ...
        (SELECT COALESCE(SUM(total_cost), 0) FROM api_logs WHERE team_id = t.id AND strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')) as monthly_cost
      FROM teams t
...
router.get('/:id/members', auth, (req, res) => {
  try {
    ...
    const members = all(`
      SELECT tm.*, u.username, u.name, u.avatar_color
...
router.put('/:id', auth, (req, res) => {
  try {
    ...
    if (!membership || membership.role !== 'owner') {
      return res.status(403).json({ error: 'Only owner can update team' });
    }
    const { name, budget_limit } = req.body;
...
router.get('/:id/costs', auth, (req, res) => {
  try {
    ...
    const costs = all(`
      SELECT u.name as user_name, u.username,
        SUM(al.total_cost) as total_cost,
        SUM(al.total_tokens) as total_tokens,
        COUNT(*) as call_count
      FROM api_logs al
      JOIN users u ON al.user_id = u.id
      WHERE al.team_id = ? AND al.created_at >= datetime('now', '-' || ? || ' days')
      GROUP BY al.user_id ORDER BY total_cost DESC
    `, [req.params.id, days]);
    ...
    const stats = get(`
      ...
      FROM api_logs WHERE team_id = ? AND created_at >= datetime('now', '-' || ? || ' days')
    `, [req.params.id, days]);
